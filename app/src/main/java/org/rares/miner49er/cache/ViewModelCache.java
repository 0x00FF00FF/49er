...
//    }
  ...
  public void dumpCaches() {
    ...
    for (ProjectData project : getProjectsLruCache().snapshot().values()) {
      Log.d(TAG, "dumpCaches: project id: " + project.getId() +
          "\nissues: " + (project.getIssues() == null ? "null" : project.getIssues().size()) +
          "\nowner: " + (project.getOwner() == null ? "null" : project.getOwner().getName()) +
...
//    Log.v(TAG, "dumpCaches: " + getIssuesLruCache().snapshot().toString());
    for (IssueData issueData : getIssuesLruCache().snapshot().values()) {
      Log.d(TAG, "dumpCaches: issue id: " + issueData.getId() + " parent id: " + issueData.getParentId());
      Log.d(TAG, "dumpCaches: timeEntries: " + (issueData.getTimeEntries() == null ? "null" : issueData.getTimeEntries().size()));
    }
