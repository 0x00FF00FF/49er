...
/**
 * Copy-pasted from {@link ListPreloader}. <br />
 * Added modifications to support double
 * image preload of multiple sizes with
 * the same request.
 * <hr />
 ...
 */
public class MultipleListPreloader<T> implements AbsListView.OnScrollListener {
     ...
     */
    public MultipleListPreloader(@NonNull RequestManager requestManager,
    ...
    private void preloadItem(@Nullable T item, int position, int perItemPosition) {
        ...
        for (int i = 0; i < /*dimensions.length*/ 2; i += 2) {
            // hope that in future version ^
            // these dimensions are actually
            // taken into account
            if (dimensions.length == 0) {
//                Log.w("Preloader: ", "preloadItem: no sizes to preload.");
                dimensions = new int[]{256, 256};
            }
//            Log.v("Preloader", "preloadItem: " + dimensions[i] + ", " + dimensions[i + 1]);
            preloadRequestBuilder.into(preloadTargetQueue.next(dimensions[i], dimensions[i + 1]));
        }
    }
...
//        @SuppressWarnings("PMD.AvoidInstantiatingObjectsInLoops")
        ...
        private PreloadTarget next(int width, int height) {
